...
| --- | --- | --- | --- |
| [References and Regions](references-and-regions.md) | **both** | Memory strategy chosen per object, with safety preserved across all of them | mechanism built, two regions ship in core and a third, reference counting with weak references, as a library package; an arena and a generational pool ship as library values, not regions an allocation names; a stop-the-world tracing collector ships as a library package, `collector`'s `gc`, with no incremental mode or barriers yet |

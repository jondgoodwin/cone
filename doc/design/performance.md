...
built yet: an arena and a generational pool exist only as library values (the
`arena` and `pool` packages) rather than as regions an allocation names, there is no

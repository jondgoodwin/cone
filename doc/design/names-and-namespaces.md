...
| `compiler/c/ir/stmt/program.c` | Owns the program's module list and the file registry, and runs name resolution in three walks: what every module's `extends` names, then every module's folds, then every module's body. |

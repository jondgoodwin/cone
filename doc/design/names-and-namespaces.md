...
exempt, since it is never a symbol. ⚠ *Refusing a private candidate behind a
public name is Jon's* [Jon 27 Aug]; *deciding it by the name rather than by the
candidates is Penny's default from the `pub` build* [Penny 19 Sep], standing
unless he objects ("Visibility", below).
...
- **Every file the compiler builds as a module opens with its `mod` line** [Jon 24 Sep] — a lone file, a module's root, a folder module's designated file, a one-file module — naming the file, or the folder; a first file without one is `ErrorNoModDcl`. The other files of a folder module carry none: the folder names the module they join. **A module's imports come right after its `mod` line**, ahead of every other declaration, **and so all of them are in its designated file** [Jon 23 Sep]: *"Imports can't happen in a sibling file because they all have to be after `mod`, right? And a sibling file doesn't have a `mod`. So there's no place to put it … all of that stuff has to be in the main designated file."* An `import` after another declaration, or anywhere in a file of the module with no `mod` line, is `ErrorImportLate`. So a module's header is comments, the `mod` line and the imports, which is all Congo reads of it.
...
- **Nesting is by files and folders only.** A module is never declared inside a source file: a `mod name { ... }` block there is recognised only to be refused, with its body skipped. The one file that writes one is a package's generated include file, which holds a private, pruned block for each submodule it reaches, so each name is spelled after its real owner [Jon 25 Sep]. A module trait is not a module and is declared inside a file, as any declaration is.
...
- A path may have any number of hops, each of which must resolve to a module or a struct-like type. **An alias resolves to whatever is at the end of its chain**, so a `typedef` of a struct is a hop like the struct itself. **A generic type's instance is a hop to its functions**, `List[i64].empty()`, bound at type check where the instance exists, with the same privacy and abstract-method rules as a hop through a named type; to any other member of an instance, and through anything else — a number type, a generic parameter — a hop is `ErrorUnkName` at type check.
...
**One operator is Jon's decision** [Jon 19 Sep], built 21 Sep when `::` retired. What a reader loses is the signal that a name crossed a namespace rather than reached into a value; he declined it for the reason he declined the underscore for visibility — a name's spelling should not carry a fact about where it came from — and the two were never told apart by eye once a path ran through both (`mymod::config.width`). Four consequences were put to him before the build, and he took the recommendation in each [Jon 21 Sep]: *"Yeah, I'm good with all four of your recommendations. Let's make it happen."* They were: a module-level name hidden by a local or a member stays unreachable until a module can name itself (the `mod` declaration later gave it that name); the first name of a path is found by ordinary lexical lookup, so `Self.make(a)` works and a local shadows a module of its spelling — a behaviour change blessed, not slid past; the "namespace X does not exist" family of messages becomes ordinary unknown-name and no-member messages, less specific because the source is; and a hop through a generic instance or a type parameter (`Box[i64].make`, `T.make`) is out of scope and refused at type check. `::` itself is left unassigned: freeing it was that change's job, and spending it is a later grammar pass's.
...
*Jon's reasons for private-by-default* [Jon 18 Sep], when `pub` replaced the underscore: the interface is what is precious — *"That's the seam, the boundary between the inside and the outside of something. And we want that seam to be as tiny as possible by default"* — so writing `pub` is the author saying "yes, this is the interface", where under the old rule a module of unmarked functions was all interface by accident. Where it costs him is struct fields: he is *"a big fan of direct access to the fields inside a struct"*, and accepted the cost anyway — *"if AI is writing the code, writing `pub` in front of field names probably is not that big a deal."* What decided it was reading nested submodules spelled `_parse`, `_ir`, `_typecheck` under the old rule: *"It just made me feel all kinds of bad."* He weighed losing the signal a marked name carries wherever it is read, and dismissed it. Penny's additions, accepted: the default is the design, since whatever is cheapest to write is what gets written; and a public field is a commitment to representation, which `pub` on a field makes a decision without requiring an accessor. That a folded or imported name is private to the module that folded it until `pub` says otherwise is his too [Jon 14 Sep]: *"a name that is public to a module is, when imported, private to the next … we need this ability to have even public names end up being private and not importable at the next level up."* And a `but` naming a private name is refused as a missing name is [Jon 23 Sep: *"option one for sure"*], since from outside a private name must behave as one that is not there.
...
**An `import` is written in its module's header** [Jon 23 Sep]: right after the `mod` line of the module's designated file, with the module's other imports and ahead of every other declaration, so a module's whole inbound dependency list sits at its own declaration. An `import` after any other declaration — a standalone `use` included — is `ErrorImportLate`, however the import is spelled; it is still made, so nothing else follows from the refusal. A file the folder swept into its module has no `mod` line, and so no place for an import: an `import` anywhere in it is `ErrorImportLate` too. **There is no placement rule for `use`** [Jon 23 Sep: *"`use` can happen wherever use or folds or whatever apply"*]: as a clause it is on a declaration, so it lives wherever that declaration lives, and each site already says where that is; as a statement it is in a type body or at module scope, where a module's folds do not depend on the order they were written in.
...
*Jon's rulings, and his words* (19–20 Sep, in designing it). **Visibility is transitive, no exceptions** [Jon 20 Sep]: *"If engine is private, and thrust is public in Engine, then engine.thrust cannot be visible. And therefore the folded thrust is equally invisible"* — so a type cannot hide the field it delegates to, and hiding is done with a hand-written method. **A collision is always an error**, under `*` as for a list [Jon 20 Sep: *"how can we do otherwise?"*], an override being spelled as an exclusion, with `but` preferred to `except`; so a base gaining a member breaks the build at the fold rather than silently overriding. **A fold grants no subtyping, because subtyping is never between two concrete types** [Jon 20 Sep]: *"is-a ties between a concrete object and its abstraction… Abstract types are traits, not structs"*; folding and extensions are two ways of raising a concrete type into compliance with a trait after the fact, so a folded method counts toward structural conformance. **A folded method is a new node, not the original bound again** [Jon 20 Sep]: *"Name folding is super straightforward on data, just namespace tinkering. Same for modules. Methods on the other hand have a significant semantic flavor… you cannot fold the original node in unchanged."* **Position lives in field nodes only** [Jon 20 Sep]: *"the method is just the function. It's not the parameter. The hop exists in the data reference, which is a field decl node"*; a chained copy points at its parent — *"C will point to B, will point to the original field A"*, a forward linked list — and *"An alias doesn't hold a field at all. Aliasing and field decls are two entirely separate nodes."* **The dictionary is built whole in name resolution** [Jon 19 Sep]: *"you want the namespace to be fully populated before you have to then ask the method, are they available."* **A folded field satisfies no field requirement** [Jon 19 Sep]: *"They don't belong to the type, they belong to the type's type. We've just provided a name folding, a referencing convenience."* **A folded method's vtable slot holds a thunk** [Jon 19 Sep]: *"that thunk is a method, it's just not in a namespace"* — so "no forwarders" holds as no forwarder is a member of the type, reference-held hops fill slots, and a program that never folds pays nothing per virtual call. **A trait may not fold** [Jon 19 Sep]: *"They are abstractions. The underlying organizing details are irrelevant."* The five small decisions taken while building it he accepted as built [Jon 22 Sep]: *"let's just say that they're done. If I have a problem, we'll come back and we'll figure it out."*
...
**A type-body `use` is never a consumer-side import** — ⚠ *Penny's default from the design session, not Jon's ruling* [Penny 22 Sep], standing unless he objects. Inside a type the word is always producer-side composition — this type is built out of that one — and ordinary lookup inside a type's bodies comes from the enclosing module, which is what `import` fed. The same word one scope out means the consumer-side thing — a module's standalone `use`, which asks for an enum's variants or a submodule's names as the module's own — and the two do not meet: a type body has no imports of its own.
...
**An `extern` global carries the clause too** [Jon 25 Sep]. The fold reads only the global's declared type, which an `extern` global has, and `extern` means only "defined in a differently compiled unit". It is how a package's include file passes on what a folded global puts into the package's namespace: `pub extern mut config Config pub use *;` beside `Config`'s declaration, and an importer reaches the folded names as it would against the source, each lowered to the package's one `config` (`module_build_fold_extern`). Where the type is out of view the refusal is the type's own, once, where it is written; the rest of the rules below apply unchanged (`module_build_fold_extern_nameres`). A hand-written include file may fold differently from its source, and nothing checks it; a generated one copies the line.

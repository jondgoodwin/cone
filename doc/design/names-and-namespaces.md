...
`module-extends` runs a chain of two and both importers, both bases' private names and the base's private fold reached from inside, and the extending module importing for itself a module the base imports; `module-extends-back` refuses a module importing the module that extends it, in both orders; `module-extends-import` extends a module an import reached by its file; `module-extends-nameres` and `module-extends-parse` pin the refusals.

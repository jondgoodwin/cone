...
> decisions and are owned by [[ownership-memory-safety|Ownership memory safety]],
> [[diagnose-instead-of-crash|Diagnose instead of crash]], [[unenforced-language-rules|Unenforced language rules]] and
> [[compiler-defect-backlog|Compiler defect backlog]]. Fourteen are pinned by `xfail` scenarios that will
...
> than filed: float intrinsics went to [[intrinsics|Intrinsics]] as future capability, and
> `<-` has a reserved `stream` row awaiting collections, see
> [[collection-streams-and-iteration|Collection - Streams & Iteration]].
...
| AST-dump assertions | Out of scope. `--ir` output has no stability contract and [[ir-refactor|IR refactor]] would invalidate any golden written against it. |
...
The [[overload-refactor|Overload Refactor]] work built a fixture suite exercising much of this by
...
pick one up: [[ownership-memory-safety|Ownership memory safety]] for the miscompiles and the soundness
hole, [[diagnose-instead-of-crash|Diagnose instead of crash]] for the error paths that dereference their
own NULL, [[unenforced-language-rules|Unenforced language rules]] for the rules nothing checks, and
[[compiler-defect-backlog|Compiler defect backlog]] for the residue. What follows stays here as the
...
exception handling to [[error-handling|Error Handling]]; concurrency and the sendability flags
to [[concurrency-threads|Concurrency Threads]] and [[concurrency-primitive-types|Concurrency Primitive Types]]; the `#`
meta-language to [[metaprogramming|Metaprogramming]]; weak references and lock permissions to
[[regions|Regions]] and [[permissions|Permissions]]; closure capture and closure references to
[[types-function-and-closure|Types. Function and Closure]]; initializers and `clone` to [[init-and-final|Init and Final]];
delegated inheritance to [[types-struct-and-union|Types. Struct and Union]]; selective import to
[[using-and-module-name-folding|Using and Module Name-folding]]; `enum` to [[types-number-and-enum|Types. Number and Enum]]; and
`typedef`/`extend` to [[type-inference-and-coercion|Type Inference and Coercion]]. `imm` enforcement, module
privacy and the lifetime gaps are rules rather than features and go to
[[unenforced-language-rules|Unenforced language rules]].
...
- The defects above, now owned by [[ownership-memory-safety|Ownership memory safety]],
  [[diagnose-instead-of-crash|Diagnose instead of crash]], [[unenforced-language-rules|Unenforced language rules]] and
  [[compiler-defect-backlog|Compiler defect backlog]]. Three with unambiguous one-line fixes — the
...
- **`.sqrt()` and the other float functions are future capability**, and belong
  ...
  [[intrinsics|Intrinsics]]. Coverage follows the capability; when it is built, R6.1's

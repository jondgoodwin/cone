...
> moved to [[regions|Regions]], which owns the design it needs. Cross-module private
> access was fixed here after all, because the language answer it was waiting for
> is already published; the corresponding entry in [[unenforced-language-rules|Unenforced language rules]]
> is closed.
...
groups" in [[add-test-suite|Add test suite]], shape B.
...
| Fallible allocation `?+rc-mut v` | Needs a rebuilt lowering, not a repair | **Moved** — see [[regions|Regions]] |

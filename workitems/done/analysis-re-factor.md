...
- **A union variant may hold its own union by value**, and the field is silently
  dropped from the layout. Carried by [[types-struct-and-union|Types. Struct and Union]], which has the
  measurement.
- **Giving the parser distinct node shapes for distinct syntaxes**, which is what
  would actually shrink `fnCallTypeCheck`. [[lexer-and-parser|Lexer and Parser]] and
  [[ir-refactor|IR refactor]].
- **Migrating the eight type properties out of `flags`** into `ITypeNodeHdr`.
  [[ir-refactor|IR refactor]]'s call; this work neither needs it nor makes it worse.
- **`assert(0)` is a no-op** in the release build. [[compiler|Compiler]] owns what

...
> answer was handed to the item that owns it: fallible allocation to [[regions|Regions]],
> array fill literals to [[types-array|Types. Array]], two loose defects to
> [[compiler-defect-backlog|Compiler defect backlog]]. The suite went from 111 scenarios / 98 passed / 14
...
handed to [[regions|Regions]], which owns the decision it needs. Tracing them uncovered
...
| Fallible allocation `?+rc-mut v` | Loud, immediately | **Moved** — the design is settled, see [[regions|Regions]] |
...
generation is the general answer**, and it is recorded in [[types-array|Types. Array]] along
...
- **Fallible allocation** goes to [[regions|Regions]], with the design settled: allocation
...
- **What an array fill literal does about a runtime element count** goes to
  [[types-array|Types. Array]], which already owns array literals. The general answer is to
...
skipped on every branch and leaks — [[copy-and-alias-vs-move-semantics|Copy & Alias vs. Move Semantics]] already
...
Weak references and lock permissions do not exist; see [[regions|Regions]] and
[[permissions|Permissions]]. `RaceSafe` and `IsLockless` are declared, populated, and read
nowhere, which belongs to [[unenforced-language-rules|Unenforced language rules]].

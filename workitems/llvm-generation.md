...
Read from source, unverified. The plainly-defective ones went to [[bugs|Bugs]] and
...
- **Two arms of `genlExpr`'s `AliasTag` case are dead.** `flowInjectAliasAmt`
  ...
  [[copy-and-alias-vs-move-semantics|Copy & Alias vs. Move Semantics]] says a destructuring's alias adjustment
...
- **The `FlagGenMod` condition reads like an inverted test.**
  ...
  [[packages-and-separate-compilation|Packages and Separate Compilation]], which owns whether it should change.

...
[[Packages and Separate Compilation]] needs that for import folding — the binding
...
[[Analysis re-factor]] made the *guarantee* available that a reached declaration
...
with [[Lexer and Parser]] giving `()` and `[]` distinct node shapes: fewer
...
- `extern` merging and code-generation provenance:
  [[Packages and Separate Compilation]], step 9.
- Selective folding, `as`, `except` and `pub` re-export:
  [[Using and Module Name-folding]], and step 5 of the same item.

...
- Switch to `()` vs. `[]`?  [[Collecting operators]]
...
## The generic parameter leak, moved to [[Bugs]]

`fnDclNameRes`, `structNameRes` and `macroNameRes` all resolve their parameter
list before `nametblHookPush()`, so a type parameter leaks into the enclosing
module namespace. Measured three ways — a valid program rejected, an invalid one
silently accepted, and a hard compiler abort on instantiation. Repros and the
fix are in [[Bugs]].

...
`module.h`'s comment, `PLAN.md` or [[__Top Priority]], that stage fixes them.
...
contents, including files the parser never reads (see [[Metaprogramming]] on
...
- Handle `genname` correctly regardless of aliasing and cross-package reference —
  the same requirement [[IR refactor]] item 2.1 states for `genericdef`.
...
[[Tag Group and Name Aliasing Refactor]], which holds the detail.
...
Detail carried by [[Using and Module Name-folding]].
...
**Exit:** [[Option]] and [[Result]] are library code, and changing either needs
no compiler rebuild.
...
- Auto-generate a C header for foreign consumers — see [[C-ABI Generation]].
...
- **Regions as library code** — [[Regions]]. Replace the
...
- **Macros as type methods** — [[Macro and Inline]], so `?`, `??`, `?.` and the
  error-handling forms on `Option` and `Result` are library-defined rather than
  parser-lowered. See [[Option]], [[Result]], [[Error Handling]].
- **Module `init` and `final`** — [[Init and Final]]. `refmodule.html` specifies
...
- **Module polymorphism** — [[Module generics and traits]]. Modtraits first: the

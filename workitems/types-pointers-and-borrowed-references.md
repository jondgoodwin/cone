...
Established by [[unenforced-language-rules|Unenforced language rules]], which closed two of the three
...
- **A borrow laundered through a variable.** `mut r &i32; r = &local; r`
  ...
  - Its control-flow join rule (`if c { r = &a } else { r = &b }` takes the
    ...
    [[copy-and-alias-vs-move-semantics|Copy & Alias vs. Move Semantics]] needs. Build them together rather than
    twice.
...
[[compiler-defect-backlog|Compiler defect backlog]].
...
1. It reverses a documented behavior. "Receivers are not auto-borrowed" is
   recorded as an established fact by [[add-test-suite|Add test suite]], and
...
[[types-function-and-closure|Types. Function and Closure]] and delegates are three views of one question —
how a callable and the thing it is called on travel together — and are worth
designing at once rather than three times.


## Three reference-node defects, fixed in [[bugs|Bugs]]
...
[[concurrency-threads|Concurrency Threads]]'s.

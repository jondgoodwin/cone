...
- ‘Copy’ method: use w/ allocated references, assignment, fn call, etc.
	- **`WarnCopy` (3003) is already declared for this and is raised nowhere**,
	  deliberately kept by [[unenforced-language-rules|Unenforced language rules]] so the intent lives where
...
Cause 1 was a plain defect and is fixed by [[bugs|Bugs]]: `flowScopeDealias`'s "do

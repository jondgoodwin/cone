...
`(pt.x).sqrt()`, and when [[add-test-suite|Add test suite]] decomposed that file into coverage
groups this was the one construct with no owning group: `core` owns number types
and [[type-inference-and-coercion|Type Inference and Coercion]] owns conversion, but neither owns the
...
`design/diagnostics/test-suite.md` records that a feature with no group means both.

...
- **Nesting is by folders only.** A module is never declared inside a file: a `mod name { ... }` block is recognised only to be refused, with its body skipped. `mod trait`, a module's abstraction, is admitted by the grammar and unbuilt.

...
| a **module** declaration — `mod solids extends shapes` | nothing; a module has one instance | an alias targeting the base's own binding, for every name of the base, as visible here as there |
...
`mod solids extends shapes;` — the module's declaration, as its designated file's first statement — makes `solids` a module that **reuses** `shapes`: every name of `shapes` becomes a name of `solids`, and `solids` adds its own declarations beside them.
...
**"Every name" is every binding of `shapes`'s namespace**, so it includes what `shapes` only imported: its private `import c` puts `c` in reach of `solids`'s code, and its private folds — the automatic wildcard import of core among them — arrive beside `solids`'s own, the same declarations and so one binding each. A declaration of `solids` spelled like something `shapes` merely imported is `ErrorExtendsOverride` too. `foldStarItems` admits the private names under `FoldAdmitBase`, and `importFoldItem` takes each alias's `FlagPub` from the base's binding rather than from a clause.
...
`module-extends` runs a chain of two and both importers, and both bases' private names reached from inside; `module-extends-back` runs a module importing, `use *`, the module that extends it, in both fold orders; `module-extends-import` extends a module an import reached by its file; `module-extends-nameres` and `module-extends-parse` pin the refusals.

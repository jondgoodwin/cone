...
| --- | --- | --- |
| Module | yes, `ModuleNode.namespace` | at **parse** time by `modAddNamedNode`, which is also where an import binds the module it names; extended by the fold pass — `importNameRes` for what the module extends and what it imports, `foldGlobalExpand` and `foldEnumUseExpand` — ahead of any module's body |
...
| `ir/stmt/import.c` | `importNameRes`, `importBindModule` | the module's own binding and each folded name, as aliases carrying the import's visibility; the source's *namespace* is what is read, and a private binding of it does not fold — except to a module that extends it, which takes every name with the base's own visibility |
| `ir/stmt/module.c` | `modFoldBind` | every fold's binding into a module's namespace — an import's, an `extends`, a global's clause, an enum's `use` — where the same declaration by the same route is the binding the name has already, public if either route is, and only a different declaration collides |

...
| | `parseAddVariant`, `parseVariantTagPin` | joining a variant to its enum: the closed flags, the synthesized base link, the tag value written or assigned in sequence, the module binding. A variant of an enum that *extends* another keeps the unassigned sentinel unless a value was written, because that enum's numbering continues from its base's last and the base is not resolved yet |
| | `parseEnumExtensionMember` | what an enum extending another may not declare — a member of any kind, since the variants it shares with its base are the base's declarations, and a field or method every variant carries belongs where those are. Nor a discriminant: none is synthesized for such an enum either, its base's arriving with the fields name resolution splices in |

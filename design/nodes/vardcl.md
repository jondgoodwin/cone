...
`AliasDclNode` (`ir/stmt/aliasdcl.c`), a local spelling and a target. It is
what a type's namespace holds for a folded method, overload set or macro
method, and it is the binding record — spelling, visibility bit, target — that
import renaming will use; `aliasDclResolve` follows a chain of them to the
declaration. It carries no `vtype`, no marks and no storage, so nothing here
about type check, flow or generation applies to it.

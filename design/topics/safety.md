...
| release of an owning reference at scope exit | **partly** | leaks on a conditionally-moved variable, for arrays of owning references, and for an `Option` holding one (a `?+` allocation) |

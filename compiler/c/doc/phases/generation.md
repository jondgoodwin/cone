...
- **A struct's drop must load after `StructGEP`.** The GEP gives `T**` for a
  ref-typed field; the release routines want the reference the field holds, so
  `genlFinalizeAt` loads it.

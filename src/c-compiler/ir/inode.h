...
#define EnumTrait          0x0008  // An enumtrait, where all implementations are padded to same size
#define EnumField          0x0010  // A trait/struct has an enumerated field identifying the variant type
...
// - pstate is helpful state info for node traversal
// - node is a pointer to pointer so that a node can be replaced
void inodeTypeCheck(TypeCheckState *pstate, INode **pgm);
